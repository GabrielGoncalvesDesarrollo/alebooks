    ...
    return (
      <IconPicker.Preview
        className={classnames(
          'art-component-icon-picker-button',
...
class IconPickerDropdown extends Component {
  ...
  getDropdownContent() {
    ...
    const rows = [
      {
        ...
        render: (
          <Fragment key="form">
            <TextControl
              label={__('Buscar Icono', '@@text_domain')}
              value={maybeDecode(this.state.search)}
              onChange={(searchVal) => this.setState({ search: maybeEncode(searchVal) })}
              placeholder={__('Escribe para buscar...', '@@text_domain')}
              autoComplete="off"
              autoFocus
            />
            <div className="art-component-icon-picker-input-output">
              {customizeIcon ? (
                <TextareaControl
                  label={__('Icono', '@@text_domain')}
                  value={value}
                  onChange={(newData) => {
                    onChange(newData);
                  }}
                  placeholder={__('Icono', '@@text_domain')}
                  autoComplete="off"
                />
              ) : (
                <Button
    ...
    eachIcons((iconsData) => {
      ...
      rows.push({
        ...
        render: (
          ...
          <div
            className={classnames(
              'components-panel__body art-component-icon-picker-list-panel-toggle',
      ...
      allIcons.forEach((icon, i) => {
        ...
        if (currentIcons.length === 3 || allIcons.length === i + 1) {
          rows.push({
            key: 'icons',
            render: <div className="art-component-icon-picker-list">{currentIcons}</div>,
          });
    ...
    const result = (
      <AutoSizer key={`autosizer-${customizeIcon ? '1' : '0'}`}>
        {({ width, height }) => (
          <List
            className="art-component-icon-picker-list-wrap"
            width={width}
            height={height}
    ...
    return (
      <Fragment>
        <div className="art-component-icon-picker-sizer" />
        <div className="art-component-icon-picker">{result}</div>
      </Fragment>
    );
  }
  ...
  render() {
    ...
    const dropdown = (
      <Dropdown
        className={className}
        contentClassName="art-component-icon-picker-content"
...
IconPicker.Preview = function (props) {
  ...
  return result || alwaysRender ? (
    <IconPicker.Render
      ...
      className={classnames(
        className,
        'art-component-icon-picker-preview',
        onClick ? 'art-component-icon-picker-preview-clickable' : ''
      )}
      onClick={onClick}
      onKeyPress={() => {}}
      role="button"
