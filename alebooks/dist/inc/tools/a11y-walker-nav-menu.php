...
class A11Y_Aria_Walker_Nav_Menu extends Walker_Nav_Menu
{
	...
	public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
	{
		...
		if (!in_array('menu-item-has-children', $classes)) {
			...
			$output .= "<li $class_names role='menuitem'><a class='am-link' href='$item->url'>$item->title</a>";
		} else {
			// trying to use a data attribute to keep track of the list items with children.
			$output .= "<li data-count='$this->parentItemCount' data-has-children='true' $class_names role='menuitem'>";
			if (!$has_top_level_page) {
				...
				$output .= "<a class='am-submenu-link' aria-label='$item->title, tab to the next button to expand the sub-menu' href='$item->url' $attributes >$item->title";
				$output .= "</a>
